...
{block name=content}
	<section class="u-mb-32">
		...
		{* 表示選択フォーム *}
		<form action="{$BASE_URL}admin/special/" method="POST" class="history__filter u-mb-24">

			{* state *}
			<input type="hidden" name="state" value="special">
		...
		<form action="{$BASE_URL}admin/special/" method="POST">
			...
			{* 特別休暇テーブル *}
			{if $special_lists|@count > 0}
				{foreach from=$special_lists item=user}
								...
								{foreach from=$user.records item=r}
									<tr class="table__tr">
										...
										<td class="table__td table__td--action">
											<a href="{$BASE_URL}admin/special/edit/{$r.id}/" class="button button--sub button--sm">修正</a>
										</td>
			...
			<div class="button__wrap">
				<a href="{$BASE_URL}admin/special/create/" class="button button--main">追加する</a>
			</div>
		</form>
	</section>
{/block}
